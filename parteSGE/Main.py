...
# pide el nombre de usuario y la contraseña
...
datos = {"username": usuario, "password": contra}  # Credenciales del encargado
...
if respuesta:
    ...
    if Encontrado == True:
        segundos = segundos + (int(incidenciaAux.getHoras()) * 3600) + (int(incidenciaAux.getMinutos()) * 60) + int(
            incidenciaAux.getSegundos())
        ...
        if segundos >= 60:
            ...
            if minutos >= 60:
                horas = minutos / 60
                minutos = minutos % 60
        incidencia = Incidencia(id, booleano, int("{:.0f}".format(horas)), int("{:.0f}".format(minutos)),
                                int("{:.0f}".format(segundos)))
